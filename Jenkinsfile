node {
  ...
  node('homolog-starbem') {
    ...
    stage('Promote to Staging') {
      version_staging = "${version}-staging"
    ...
    stage('Start to Production') {
      userInput = input(id: 'Proceed1', message: 'Start to production?', parameters: [])

      build job: 'node-api-prisma-boilerplate-production', parameters: []
    }
  }
}
